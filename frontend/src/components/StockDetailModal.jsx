...
export default function StockDetailModal({ stockId, onClose }) {
  ...
  return (
    <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal-box" style={{ width: 700, maxWidth: '95vw', maxHeight: '88vh', overflowY: 'auto', padding: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid #e2e8f0', position: 'sticky', top: 0, background: '#fff', zIndex: 1, borderRadius: '16px 16px 0 0' }}>
          {cfg && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 16, fontWeight: 800 }}>{cfg.name} <span style={{ fontSize: 11, color: '#64748b', fontWeight: 500 }}>{stockId}</span></div>
            ...
            {state === 'content' && stockData?.price && (
              <div style={{ textAlign: 'right' }}>
                ...
                {stockData.changePct !== null && stockData.changePct !== undefined && (
                  <div style={{ fontSize: 11, color: stockData.changePct >= 0 ? '#16a34a' : '#dc2626' }}>
                    {stockData.changePct >= 0 ? '▲' : '▼'} {Math.abs(stockData.changePct).toFixed(2)}%
                  </div>
                )}
              </div>
            )}
        ...
        <div style={{ padding: '20px' }}>
          {state === 'loading' && (
            <div style={{ textAlign: 'center', padding: '50px 0', color: '#64748b', fontSize: 14 }}>데이터를 불러오는 중...</div>
          )}
            ...
            <>
              <div className="grid g21 mb">
                <div className="other-card">
                  ...
                  <svg viewBox="0 0 500 150" width="100%" height={140} style={{ display: 'block' }} dangerouslySetInnerHTML={{ __html: chartSvg }} />
