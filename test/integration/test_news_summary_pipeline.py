#!/usr/bin/env python3
"""
뉴스 데이터 로더 + 프롬프트 매니저 통합 테스트

실제 로컬 뉴스 데이터를 로드하고 LLM으로 요약하는 파이프라인을 테스트합니다.
"""
...
def main():
    """뉴스 로더 + 프롬프트 관리 시스템 테스트"""
    logger.info("=" * 60)
    logger.info("뉴스 데이터 로더 + 요약 파이프라인 테스트")
    logger.info("=" * 60)
    ...
    logger.info("=" * 60)
    logger.info("\n통합 워크플로우:")
    logger.info("  1. NewsDataLoader: 로컬 뉴스 데이터 로드")
    logger.info("  2. 날짜 범위 필터링")
    logger.info("  3. PromptManager: 프롬프트 렌더링")
    logger.info("  4. GatewayClient: MLflow AI Gateway 호출")
    logger.info("  5. LLM 요약 수신")
