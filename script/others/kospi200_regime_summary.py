"""
KOSPI200 가격 국면별 뉴스 LLM 요약

S3 경로: s3://fisa-news-archive/preprocessed/KOSPI200/{yyyy}/{mm}/{yyyy-mm-dd}.json
출력:    data/KOSPI200/regime_news_summary_KOSPI200.json

실행:
    python script/kospi200_regime_summary.py
    python script/kospi200_regime_summary.py --provider gemini
    python script/kospi200_regime_summary.py --start 2020-01-01 --end 2026-05-12
    python script/kospi200_regime_summary.py --dry-run
"""
...
# ══════════════════════════════════════════════════════════════════════
TICKER_NAME = "코스피200"
TICKER_CODE = "KOSPI200"
FDR_CODE    = "KS200"       # FinanceDataReader 코드
SECTOR      = "시장지수"
...
S3_PREFIX = "preprocessed/KOSPI200"   # 개별 종목과 다른 경로
...
주어진 KOSPI200 지수 국면(기간, 방향, 거래량 추세)의 이동 원인을
뉴스를 종합해 분석합니다.
반드시 JSON 형식으로만 응답하세요. 설명 텍스트는 포함하지 마세요."""

_USER_TMPL = """\
{start} ~ {end} ({days}일간) KOSPI200 지수가 {dir_str} {cum_ret:+.1%} 이동했습니다.

■ 구간 요약
  · 방향: {direction}  누적 등락률: {cum_ret:+.2%}
...
위 정보를 바탕으로 이 구간의 지수 이동 원인을 분석하고 아래 JSON 으로만 응답하세요:
{{
  "cause":       "주요 원인 한 줄 (100자 이내)",
  "evidence":    [
    {{"date": "YYYY-MM-DD", "quote": "원문 핵심 구절 직접 인용 (최대 40자)", "point": "이 구절이 지수 변동과 연결되는 이유 (50자 이내)"}},
...
def _fetch_price_volume(start: str, end: str) -> tuple[pd.Series, pd.Series]:
...
# ── S3 뉴스 수집 (preprocessed/KOSPI200 전용 경로) ────────────────────

def _fetch_regime_news(s3_client, start: pd.Timestamp, end: pd.Timestamp,
    ...
    while cur <= fin:
        key = (
            f"{S3_PREFIX}/{cur.year}/{cur.month:02d}"
        ...
        try:
            ...
            for item in items:
                uid = f"{item.get('pub_date','')}|{item.get('title','')}"
                if uid not in seen:
                    seen.add(uid)
...
# ── LLM 호출 (regime_news_summary.py와 동일) ──────────────────────────
...
def run(provider: str = "groq", model: str | None = None,
    ...
    # ── 국면 도출 ─────────────────────────────────────────────────────
    log.info(f"가격 데이터 수집: {TICKER_NAME}({FDR_CODE})  {start} ~ {end}")
    returns, volume = _fetch_price_volume(start, end)
    ...
    for i, reg in enumerate(regimes, 1):
        ...
        news_articles = _fetch_regime_news(
            s3_client, reg["start"], reg["end"], NEWS_PRE, NEWS_POST
        )

        log.info(
            f"[{i:>3}/{len(regimes)}] {start_str}~{end_str} ({reg['days']:>2}일) "
        ...
        if missing:
            log.warning(f"  필수 키 누락 {missing} — 건너뜀")
            continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="KOSPI200 국면별 뉴스 LLM 요약")
    ...
    parser.add_argument("--rerun-ids", nargs="+", type=int, default=[],
                        metavar="ID", help="강제 재처리할 regime_id")
