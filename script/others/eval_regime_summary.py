...
def _extract_llm_text(llm_analysis: dict) -> str:
    ...
    ev_text  = " ".join(
        f"{e.get('quote', '')} {e.get('point', '')}" for e in evidence
    )
    parts = [
...
def run(
    ...
    summary_path = summary_path or Path(f"data/{ticker_code}/regime_news_summary_{ticker_code}.json")
    output_path  = output_path  or Path(f"data/{ticker_code}/eval_regime_summary_{ticker_code}.json")
