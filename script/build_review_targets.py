...
def run(ticker_code: str = "005930") -> None:
    ...
    # data/{ticker_code} 하위 폴더에 파일이 없으면 data 바로 하위 폴더에서 조회
    if not eval_path.exists() and Path(f"data/eval_regime_summary_{ticker_code}.json").exists():
        eval_path = Path(f"data/eval_regime_summary_{ticker_code}.json")
    if not summ_path.exists() and Path(f"data/regime_news_summary_{ticker_code}.json").exists():
        summ_path = Path(f"data/regime_news_summary_{ticker_code}.json")
        out_path  = Path(f"data/review_targets_{ticker_code}.json")
