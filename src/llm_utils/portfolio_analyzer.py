...
MLflow Prompt Registry 계약 (프롬프트는 MLflow UI 에서 직접 등록/관리):
  ...
  - 출력 JSON 선택 키: sources — '관련 최신 뉴스' 링크 [{ticker, title, url}].
    게이트웨이가 web search server tool 을 거부하므로 news_evidence_client 가
    OpenRouter 를 직접 호출해 채운다(실패 시 키 미존재 → 프론트는 섹션 생략).
...
from src.llm_utils.news_evidence_client import find_news_evidence
...
def analyze_portfolio(
    ...
    try:
        ...
        analysis, evidence_holdings = _analyze_portfolio_traced(user_id, holdings, db)

        # 근거용 '관련 최신 뉴스' 링크 — 게이트웨이가 web search server tool 을 거부하므로
        # OpenRouter 를 직접 호출하며, 별도 experiment(portfolio_news_evidence)의 독립
        # trace 로 기록한다. 분석 본문 trace 밖에서 호출해야 child span 으로 흡수되지 않는다.
        # 분석 결과(analysis)를 입력으로 넘겨 그 분석을 뒷받침하는 뉴스를 찾는다.
        # 실패해도 분석 본문은 유지되도록 find_news_evidence 가 빈 리스트로 degrade 한다.
        sources = find_news_evidence(evidence_holdings, analysis=analysis)
        if sources:
...
def _analyze_portfolio_traced(
    ...
    # 근거 뉴스 검색에 쓸 종목(현금성 제외)을 함께 반환한다. 실제 검색은 별도
    # experiment 의 독립 trace 로 남기기 위해 이 (portfolio_analysis) trace 밖에서 수행한다.
    evidence_holdings = [
        {"ticker": w["ticker"], "name": w["name"]}
        for w in aggregation["weights"]
        if w["ticker"] not in CASH_IDS
    ]
    return parsed, evidence_holdings
